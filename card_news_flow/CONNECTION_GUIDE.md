...
특정 페이지에 사용자가 만든 이미지를 넣고 싶으면 `00 카드뉴스 요청 입력`의 고급 입력 `페이지 이미지 대체 JSON`에 아래처럼 넣습니다.
기본값인 `render_mode: "content_area"`는 기존 카드 템플릿의 중앙 내용 영역에 이미지를 자연스럽게 붙여 넣습니다.
카드 전체를 이미지로 완전히 대체하고 싶을 때만 `render_mode: "full_card"`를 사용합니다.
...
서버 환경에서는 사용자의 PC 로컬 경로를 직접 쓰지 말고 `10 업로드 캐릭터 이미지 자산 등록`, `11 페이지 이미지 대체 업로드` 노드의 `업로드 이미지 파일` FileInput에 이미지를 직접 업로드합니다.
업로드 이미지는 Flow 내부에서 base64 data URI로 변환되어 뒤 노드로 전달됩니다.
별도 ImageToBase64 컴포넌트를 쓰는 환경이라면 그 Base64 Message 출력을 `Base64 Message/File 출력` 입력에 연결해도 됩니다.
...
| 2 | 10 업로드 캐릭터 이미지 자산 등록 | 자산 등록 payload | 03 캐릭터 자산 불러오기 | 카드뉴스 payload |

`10 업로드 캐릭터 이미지 자산 등록` 노드 안의 `업로드 이미지 파일`에 PNG/JPEG/WebP를 직접 업로드합니다.
이미 ImageToBase64 같은 변환 노드를 쓰고 있다면 `Base64 Output` Message를 `10`의 `Base64 Message/File 출력`에 연결합니다.

특정 페이지의 내용 영역에 완성 이미지를 넣을 때:
...
`11 페이지 이미지 대체 업로드` 노드 안의 `업로드 이미지 파일`에 완성 이미지를 직접 업로드합니다.
이미 ImageToBase64 같은 변환 노드를 쓰고 있다면 `Base64 Output` Message를 `11`의 `Base64 Message/File 출력`에 연결합니다.
기본 `이미지 배치 방식`은 `content_area`입니다. 전체 카드 대체가 필요하면 `full_card`로 바꿉니다.
