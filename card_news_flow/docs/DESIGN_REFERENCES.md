...
- `topbar`: 시리즈명과 현재 페이지 번호를 고정 표시
- `content_area`: 매월 바뀌는 제목, 본문, bullet과 내부 디자인 블록을 배치하는 중앙 정보 영역
- `character_area`: 등록된 하냥이/하댕이 `asset_id` 중 하나만 선택해 반복 사용하는 캐릭터 영역
- `action_area`: 이전/다음/CTA 버튼을 고정 위치에 표시하는 하단 동작 영역

LLM은 바깥 프레임을 새로 디자인하지 않지만, `content_area` 안에서는 허용된 `content_blocks`로 화면을 예쁘게 구성할 수 있습니다. 특정 페이지가 이미지 대체로 지정되면 해당 장은 슬롯을 만들지 않고 업로드된 이미지를 카드 전체에 맞춰 보여줍니다.

허용되는 `content_blocks`:

- `lead`: 한 줄 리드 문장
- `highlight`: 핵심 메시지 강조 박스
- `mini_cards`: 2-3개 정보 카드를 나란히 배치
- `steps`: 절차/흐름을 번호 단계로 표현
- `checklist`: 실행 항목 체크리스트
- `quote`: 짧은 인용/원칙 문장
- `metric`: 수치나 키워드 강조
- `tag_row`: 핵심 키워드 칩 나열
